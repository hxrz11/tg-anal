...
async def save_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await db.users.update_one(
        ...
        {"$set": {"user_id": doc["sender_id"]}},
        upsert=True,
    )
    await db.chats.update_one(
        {"chat_id": message.chat_id},
        {"$set": {"chat_id": message.chat_id, "title": message.chat.title}},
        upsert=True,
    )
...
async def admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS:
        return
    keyboard = [
        [InlineKeyboardButton("Chats", callback_data="chats"), InlineKeyboardButton("Stats", callback_data="stats")],
        [InlineKeyboardButton("Send", callback_data="send"), InlineKeyboardButton("Pin", callback_data="pin")],
        [InlineKeyboardButton("Summary", callback_data="summary"), InlineKeyboardButton("Users", callback_data="users")],
    ]
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    if data == "chats":
        rows = await db.chats.find().to_list(None)
        text = "\n".join(f"{r['chat_id']} — {r.get('title', '')}" for r in rows)
        ...
        await query.edit_message_text("Отправьте: summary <chat_id> <YYYY-MM-DD> <YYYY-MM-DD>")


async def send_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS:
        return
    match = re.match(r"^send (-?\d+) (.+)$", update.message.text)
    if not match:
        return
...
async def send_and_pin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS:
        return
    match = re.match(r"^pin (-?\d+) (.+)$", update.message.text)
    if not match:
        return
    chat_id = int(match.group(1))
    text = match.group(2)
...
async def summary_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS:
        return
    match = re.match(r"^summary (-?\d+) (\d{4}-\d{2}-\d{2}) (\d{4}-\d{2}-\d{2})$", update.message.text)
    if not match:
        return
    chat_id = int(match.group(1))
...
async def create_checklist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS:
        return
    match = re.match(r"^checklist (-?\d+) (.+)$", update.message.text)
    if not match:
        return
    chat_id = int(match.group(1))
    raw = match.group(2)
...
async def close_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type == "private":
        return
    match = re.match(r"^done (\d+)$", update.message.text)
    if not match:
        return
    ...
    await db.checklists.update_one({"_id": checklist["_id"]}, {"$set": {"items": checklist["items"]}})
    await context.bot.edit_message_text(
        format_checklist(checklist["items"]),
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex(r"^send (-?\d+) (.+)$"), send_message))
    app.add_handler(MessageHandler(filters.Regex(r"^pin (-?\d+) (.+)$"), send_and_pin))
    app.add_handler(MessageHandler(filters.Regex(r"^summary (-?\d+) (\d{4}-\d{2}-\d{2}) (\d{4}-\d{2}-\d{2})$"), summary_cmd))
    app.add_handler(MessageHandler(filters.Regex(r"^checklist (-?\d+) (.+)$"), create_checklist))
    app.add_handler(MessageHandler(filters.Regex(r"^done (\d+)$"), close_task))
