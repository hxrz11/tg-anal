...
async def send_and_pin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    if not user or user.id not in ADMIN_IDS:
        return
...
async def summary_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    texts = [doc["text"] async for doc in cursor if doc.get("text")]
    if not texts:
...
async def create_checklist(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    if not user or user.id not in ADMIN_IDS:
        return
...
async def close_task(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not match:
        return
    checklist = await db.checklists.find_one({"chat_id": chat.id})
    if not checklist:
        return
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.Regex(r"^send (-?\d+) (.+)$"), send_message))
    application.add_handler(MessageHandler(filters.Regex(r"^pin (-?\d+) (.+)$"), send_and_pin))
    ...
    application.add_handler(MessageHandler(filters.Regex(r"^checklist (-?\d+) (.+)$"), create_checklist))
    application.add_handler(MessageHandler(filters.Regex(r"^done (\d+)$"), close_task))
